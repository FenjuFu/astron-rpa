...
**A:** ✅ **可以。** 官方大量测试就跑在云桌面环境上（如腾讯云桌面）。

需要注意：
- **系统版本**：建议 Windows 10 1809 及以上版本，过低的系统版本可能出现登录卡住、界面异常等问题。
- **本地能跑、云上不能跑**时，优先按下面顺序排查：
  1. 确认云主机系统版本是否满足上面的要求；
  2. 取客户端日志 `%APPDATA%\astron-rpa\logs`（或安装目录下 `data\logs`）；
  3. 日志为空往往说明进程根本没起来，请携带系统版本 + 安装方式在 issue 或社群中反馈。

### Q: 🆕 手工构建客户端报错 `Cannot compute electron version from installed node modules`？

**A:** 这是 `electron-builder install-app-deps` 阶段的报错，说明 `packages/electron-app` 下的 Electron 依赖没有装全，通常不是代码问题，而是构建环境不完整。

**排查清单：**
1. **确认 pnpm 已安装且在 PATH 中**（最常见原因）：`pnpm --version` 能正常输出；仅安装未配置环境变量时，构建脚本会一闪而过或中途失败。
2. **确认 Node.js 版本 ≥ 22、pnpm ≥ 9**，详见 [`BUILD_GUIDE.zh.md`](./BUILD_GUIDE.zh.md)。
3. **先装依赖再构建**：确保前端依赖安装步骤成功执行，再执行构建。
4. 构建客户端**不需要 Java 和 Docker**，缺少这两项不影响客户端构建。

> 💡 如果直接双击运行构建脚本窗口一闪而过，改到项目根目录用命令行执行 `./build.bat -p xxx -s xxx`，这样能看到完整报错。
...
### Q: 🆕 团队多人编辑同一个流程，怎么保证版本一致？有类似 Git 的合并吗？

**A:** 目前**没有 Git 式的分支合并**，但可以通过内置机制做到团队统一：

1. **工作流版本管理**：流程支持发布新版本、回滚到历史版本、发布时做差异比对，避免"每个人电脑上一个版本"。
2. **中心化分发**：把测试通过的稳定流程发布出去，团队成员统一从中心获取和执行，而不是互相拷贝本地文件。
3. **协作约定**：同一流程尽量避免多人同时改；较大流程建议拆成组件/子流程，按模块分工，降低冲突面。

### Q: 🆕 `route.exe` 等 Go 编译产物的源码在哪里？

**A:** 部分 Go 组件的源码位于 `dev` 分支，切换到该分支即可查看。如果在 `main` 分支没找到对应目录，请先确认当前分支。

### Q: 🆕 多个客户端连接到同一个服务端时，通过 API 运行工作流如何指定在哪个客户端执行？

**A:** 通过 **API Key** 区分。开放接口的工作流执行入口（`/openapi` 下的 `execute` / `execute-async`）会用调用时携带的 API Key 解析出对应账号，工作流就在该 **API Key 所属账号登录的客户端**上执行。

- 想在不同客户端执行：为每个客户端使用其登录账号各自的 API Key，用哪个 Key 调用就在哪个客户端跑。
- API Key 在客户端 / 平台的开放接口页面获取。
- 服务端的**远程计划任务**由调度器分发到已连接的执行器，不通过上述 API Key 指定。
...
### Q: 🆕 计划任务（定时任务）到点没有触发怎么办？

**A:** 先区分是"任务没触发"还是"触发了但执行失败"，两者日志表现不同。
...
**2. 常见原因：调度宽限期过短**
定时任务基于 APScheduler 的 cron 触发器执行，默认的错过容忍时间很短（约 1 秒）。当机器负载高、休眠唤醒或时间片被抢占导致错过这个窗口时，这一次调度会被判定为 misfire 而跳过，因此表现为"前几天都正常，某天夜里没跑"。

**3. 可以先尝试的缓解手段：**
- 改用**高级 cron 表达式**配置触发时间，表达能力更强也更稳定；
- 避开整点、整分的系统高负载时段，错开多个任务的触发时间；
- 确认执行时段内机器没有锁屏、休眠或关闭客户端；
...
### Q: 🆕 私有化部署后客户端登录失败（自动登录正常、点「登录」报错），或改了组织名重启后被还原？

**A:** 多数是 **Casdoor 组织配置** 引起的。开源版通过 Casdoor 做认证，但**只使用其中一个非 built-in 组织**（初始化自带的 `example-org`，显示名为「示例组织」），且**开源版不支持多租户 / 多组织**。

1. 登录 Casdoor 管理后台（默认 8000 端口），**只保留一个非 built-in 组织**，删除自己额外新建的组织后重试登录。
2. 不要在 Casdoor 里新建额外组织，平台默认映射的就是初始化的那个组织。
3. `example-org` 的**显示名**可以改，但重启后可能回到默认值——它是内部占位组织，终端用户看不到，不影响使用，无需纠结。
